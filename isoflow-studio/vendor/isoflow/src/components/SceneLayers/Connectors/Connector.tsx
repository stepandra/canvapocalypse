...
export const Connector = ({
  ...
  const motionPathString = useMemo(() => {
    return connector.path.tiles.reduce((acc, tile, index) => {
      const point = `${tile.x * UNPROJECTED_TILE_SIZE + drawOffset.x},${
        tile.y * UNPROJECTED_TILE_SIZE + drawOffset.y
      }`;
      return `${acc}${index === 0 ? 'M' : ' L'} ${point}`;
    }, '');
  }, [connector.path.tiles, drawOffset]);
  ...
  const flowPacket = isFlowActive ? (
    <circle
      r={Math.max(connectorWidthPx * 0.88, 3.5)}
      fill={getColorVariant(color.value, 'dark', { grade: 3 })}
      stroke={theme.palette.common.white}
      strokeWidth={Math.max(connectorWidthPx * 0.26, 1.25)}
      className="isoflow-flow-packet"
    >
      <animateMotion
        dur="2.2s"
        begin={`${flowDelay}s`}
        repeatCount="indefinite"
        path={motionPathString}
        calcMode="spline"
        keyTimes="0;1"
  ...
  if (packetOnly) {
    return (
      <Box
        ...
        aria-hidden="true"
        data-flow-packet-layer={connector.id}
      >
        <Svg
          style={{
            transform: 'scale(-1, 1)',
  ...
  return (
    <Box
      style={css}
      sx={{
        opacity: isContextActive && !isFlowActive ? 0.12 : 1,
      ...
      >
        ...
        {isFlowActive && (
          <polyline
            ...
            strokeWidth={connectorWidthPx * 0.96}
            strokeLinecap="round"
            strokeLinejoin="round"
            fill="none"
            opacity={0.58}
            className="isoflow-flow-active"
          />
        )}
        <polyline
          points={pathString}
